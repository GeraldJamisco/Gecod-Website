...
if (isset($_POST['add_new_beneficiary'])) {
    ...
    if (!empty($_FILES[$fileField]['name']) && basename($_FILES[$fileField]['name']) !== '') {
        ...
        if (move_uploaded_file($_FILES[$fileField]['tmp_name'], '../img/beneficiaries/' . $filename)) {
            $conn->query("INSERT INTO gecodorphans(orphanid, orphanNames, orphanInfo, orphanImage, orphanBirthday, orphanGender)
                          VALUES('$r_string','$benefNames','$benefBio','$filename','$dateob','$gendaf')");
